    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            if user.usuario_tipo == 'operador':
                return redirect('index')  # Redirigir al usuario operador a la página de iniciocli.html
...
def actualizar(request, user_id):
    ...
    if request.method == 'POST':
        ...
        user.sectores = request.POST.get('sectores')
        user.numero_medidor = request.POST.get('numero_medidor')
        user.save()
...
def eliminar(request, user_id):
    if request.method == 'POST':
        ...
        try:
            usuario = Usuario.objects.get(id=user_id, email=correo, usuario_tipo='cliente')
            usuario.delete()
...
def crear_preferencia(request):
    # Reemplaza 'TU_ACCESS_TOKEN_AQUI' con tu access token real de Mercado Pago
    access_token = 'TEST-6996636269531487-102516-34c170d4d2bbcb5b5c5b8f54a0b45b66-543527070'
    ...
    return render(request, "pago/mercadopago.html", {"preference_id": preference["id"]})


#def mercadopago(request):
#    return render(request, "pago/mercadopago.html")
...
def agregar_consumo(request, user_id):
    user = Usuario.objects.get(id=user_id)

    if request.method == 'POST':
        # Solo permitir la edición de "consumo" para operadores
        if request.user.usuario_tipo == 'operador':
            user.consumo = request.POST.get('consumo')
        user.save()
        return redirect('lista')

    datos = {'usuario': user}
    return render(request, "operador/agregar_consumo.html", datos)
